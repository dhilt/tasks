...
let processElementsAsync = () => {
	indexes.forEach(index => 
		getMessageAsync(index)
			.then(result => elements['status' + index].innerHTML = result)
			.catch(result => {
				elements['status' + index].innerHTML = result;
				elements['status' + index].style.color = 'red';
		})
	);
};
